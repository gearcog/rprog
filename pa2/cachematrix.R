...
cacheMatrixUnitTests <- function() {
    ...
    if (is.list(cm)) {
        ...
        if (!is.function(cm$set)) {
            stop("    [FAILED] - set function not implemented")
        }
        message("    [PASSED]")
        message(" - 1c: get function")
        if (!is.function(cm$get)) {
            stop("    [FAILED] - get function not implemented")
        }
        message("    [PASSED]")
        message(" - 1d: set inverse function")
        if (!is.function(cm$setinverse)) {
            stop("    [FAILED] - setinverse function not implemented")
        }
        message("    [PASSED]")
        message(" - 1e: get inverse function")
        if (!is.function(cm$getinverse)) {
            stop("    [FAILED] - getinverse function not implemented")
        }
        message("    [PASSED]")
    } else {
    ...
    if (is.matrix(inv_m2) && dim(inv_m1) == dim(inv_m2) && 
        all(inv_m1 == inv_m2))
    {
        message("    [PASSED]")
    } else {
    ...
    if (is.matrix(inv_tm2) && is.matrix(inv_tm3) && 
        dim(inv_tm1) == dim(inv_tm2) && dim(inv_tm1) == dim(inv_tm3))
    {
        message("    [PASSED]")
    } else {
